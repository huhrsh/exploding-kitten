...
function Leaderboard({setLeaderboard }) {
    ...
    return (<>
        {loading ? <PuffLoader color="#f97316" className="absolute top-[45vh] left-[50vw] -translate-x-[50%] -translate-y-[50%] " /> : <section className="animate__animated animate__fadeInUp w-screen h-screen p-12 pt-20 z-10 fixed top-0 left-0 bg-[#000000aa]" >
            ...
            <ul className="flex flex-col" >
                ...
                {leaderboardData && leaderboardData.map((item, index) => (
                    <li className={`flex justify-between items-center w-full px-3 py-2 text-xl`} key={index}>
                        ...
                        <p className="w-1/4 p-2 ">{item.name}</p>
